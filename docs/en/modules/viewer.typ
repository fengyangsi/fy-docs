...
The three files are shipped byte-for-byte: no bundler and no transpiler stand between the repository and the browser, so the source a reviewer reads is the artifact a user runs. The shell these files bind is `assets/doc.html`, specified by the `page` chapter. That shell's `id` and `class` surface is an internal invariant, so `viewer.js` is entitled to query any of it without defending against an absent element. `tests/contract.rs` checks the direction that makes this safe: every `id` and class name bound here is scanned against the names the shell and this crate's chrome markup write, and every callout kind and badge state the template can emit is scanned against the stylesheet.
