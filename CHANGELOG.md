...
- **Page surface is checked by a static scan**: `tests/contract.rs` reads the four writers of a page's `id` and `class` surface — the shell, this crate's chrome markup, the fy-spec template expanded over every callout kind and badge state it can concatenate, and the classes `viewer.js` assigns at runtime — against its two readers, `base.css` and the selectors `viewer.js` matches, and separately checks that every id `viewer.js` binds is emitted somewhere. No browser, no typst and no new dependency: the scan is plain text.
...
- **Wide screens buy width for the material that overflows**: the document column was 820 px regardless of viewport, so a long line of code scrolled sideways inside its own box while an ultrawide display sat empty on both sides. From a 1280 px viewport the page grows to 1180 px and from 1800 px to 1440 px, while prose, headings and callout boxes stay pinned to the same 756 px measure; code blocks, figures and exported SVGs alone take the new room.
- **Stylesheet names every kind it can be asked to draw**: `base.css` now carries an accent rule for all ten callout kinds the template emits. The seven exposition kinds resolve to the value `fy-box` already handed them, so no page repaints; what changed is that a new kind can no longer arrive unstyled, which the `fy-spec` chapter already called a defect rather than a preference.
- **Class contract wording matches the scan**: the `fy-spec` invariant now names every writer and every reader of the surface instead of only the template and the stylesheet, the `viewer` chapter records that its binding set is checked against what the shell emits, and the `page` chapter states the consequence of compile-time embedding — an edit under `assets/` reaches a page only through a rebuilt binary, and the `.typ`-only dev watch set never refreshes one.
...
- **Sidebar highlight follows the reader**: the marked entry was the open chapter's own anchor and nothing else, so reading through `3.1`, `3.2`, `3.3` kept `3` highlighted while its subsections never lit, not even when clicked directly. The heading at the top of the content pane now carries `fy-toc-active` while its chapter keeps `fy-toc-chapter-active`, and a section too short to ever reach the top still lights up once the pane is scrolled to its end.

### Removed
- **Dead stylesheet weight and two dead hooks**: `base.css` still carried the retired index page — `.fy-index`, `.fy-index-head`, `.fy-brand`, `.fy-sub`, `.fy-doc-list`, `.fy-card*`, `.doc-link`, `.doc-name`, `.doc-title`, `.doc-status`, `.fy-theme-select` — while the routing landing page emits inline styles and no class attribute at all. Two classes that markup wrote and nothing read are gone as well: `fy-lang-menu`, whose element is positioned by the shared `fy-theme-menu` rule and reached by id, and `fy-error-lead`.
